...
"""
padding=True
(1)自动把【当前一个批次 (batch)】里所有长短不一的样本，填充到「这个批次中最长样本」的长度
(2)不填充，会直接报错
(3)这样设计的目的：张量运算必须是规则的数组，才能生成张量，而 GPU 基于张量计算
(4)自动填充 input_ids：自动给短样本（100）末尾补 pad_token_id
(5)自动填充 attention_mask：填充符为 0
(6)自动填充 labels：填充符 -100
(7)非法（长短不一）：无法生成张量
    [
        [1,2,3],      # 长度3
        [4,5,6,7,8]   # 长度5
